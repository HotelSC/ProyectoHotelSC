...
const AsignacionMat = sequelize.define('asignacionmaterial', {
    fechaAsignacion: DataTypes.STRING,
    fechaEntrega: DataTypes.STRING,
    observaciones: DataTypes.STRING,
    idInventario: DataTypes.STRING,
    idEmpleado: DataTypes.STRING,
}, { tableName: 'asignacionmaterial', timestamps: false });

const router = express.Router();

router.get('/', function (req, res, next) {
    sequelize.sync().then(() => {
        AsignacionMat.findAll()
            .then(asignacionMat => res.json({
                ok: true,
                data: asignacionMat
            }))
...
router.get('/:id', function (req, res, next) {
    sequelize.sync().then(() => {
        AsignacionMat.findOne({ where: { id: req.params.id } })
            // .then(Alumne => Alumne.get({plain: true}))
            .then(asigMat => res.json({
                ok: true,
                data: asigMat
            }))
            .catch(error => res.json({
